...
interface Elements {
  jobGrid: HTMLElement | null;
  savedList: HTMLElement | null;
  companySelector: HTMLElement | null;
  roleSelector: HTMLElement | null;
  companyDropdown: HTMLElement | null;
  roleDropdown: HTMLElement | null;
  postedSelect: HTMLSelectElement | null;
  keywordInput: HTMLInputElement | null;
  sortSelect: HTMLSelectElement | null;
  resultSummary: HTMLElement | null;
  jobCount: HTMLElement | null;
  savedCount: HTMLElement | null;
  jobTypeButtons: HTMLElement | null;
  clearFilters: HTMLElement | null;
  toast: HTMLElement | null;
  applicationModal: HTMLElement | null;
  modalBackdrop: HTMLElement | null;
  modalClose: HTMLElement | null;
  applicationForm: HTMLFormElement | null;
  modalJobTitle: HTMLElement | null;
  modalJobId: HTMLInputElement | null;
  applicantName: HTMLInputElement | null;
  applicantEmail: HTMLInputElement | null;
  applicantPhone: HTMLInputElement | null;
  applicantExperience: HTMLInputElement | null;
  applicantMessage: HTMLTextAreaElement | null;
...
const elements: Elements = {
    ...
    postedSelect: document.getElementById('postedSelect') as HTMLSelectElement,
    keywordInput: document.getElementById('keywordInput') as HTMLInputElement,
    ...
    applicationForm: document.getElementById('applicationForm') as HTMLFormElement,
    modalJobTitle: document.getElementById('modalJobTitle'),
    modalJobId: document.getElementById('modalJobId') as HTMLInputElement,
    applicantName: document.getElementById('applicantName') as HTMLInputElement,
    applicantEmail: document.getElementById('applicantEmail') as HTMLInputElement,
    applicantPhone: document.getElementById('applicantPhone') as HTMLInputElement,
    applicantExperience: document.getElementById('applicantExperience') as HTMLInputElement,
    applicantMessage: document.getElementById('applicantMessage') as HTMLTextAreaElement,
...
function persistSavedJobs(): void {
...
const companies: string[] = Array.from(new Set(jobs.map((job) => job.company))).sort();
const roles: string[] = Array.from(new Set(jobs.map((job) => job.title))).sort();

function renderOptions(): void {
    // no native select rendering needed for custom dropdowns
}

function renderDropdownOptions(type: 'company' | 'role'): void {
    ...
    dropdown.querySelectorAll('.custom-dropdown-item').forEach((button) => {
        button.addEventListener('click', () => {
            ...
            if (type === 'company') {
                state.company = value;
                const display = elements.companySelector?.querySelector('.select-value') as HTMLElement;
                if (display)
                    display.textContent = value;
            }
            else {
                state.role = value;
                const display = elements.roleSelector?.querySelector('.select-value') as HTMLElement;
                if (display)
                    display.textContent = value;
            }
            renderJobs();
...
function showDropdown(type: 'company' | 'role'): void {
    hideDropdown();
...
function renderJobs(): void {
    const filtered = jobs
        ...
        .sort(sortJobs);
    const jobGrid = elements.jobGrid;
    if (!jobGrid) return;
    jobGrid.innerHTML = '';
    if (filtered.length === 0) {
        jobGrid.innerHTML = '<div class="empty-state">No jobs match the selected filters. Try resetting or broadening your search.</div>';
    }
    else {
    ...
    if (elements.resultSummary) elements.resultSummary.textContent = `Showing ${filtered.length} of ${jobs.length} jobs`;
    if (elements.jobCount) elements.jobCount.textContent = String(filtered.length);
    attachCardListeners();
}

function filterByKeyword(job: Job): boolean {
    if (!state.keyword)
        return true;
...
function filterByCompany(job: Job): boolean {
    return !state.company || job.company === state.company;
}

function filterByRole(job: Job): boolean {
    return !state.role || job.title === state.role;
}

function filterByPostedRange(job: Job): boolean {
...
function filterByType(job: Job): boolean {
...
function sortJobs(a: Job, b: Job): number {
    ...
    const salaryValue = (salary: string) => Number(salary.replace(/[^0-9]/g, ''));
...
function isRecentlyPosted(rawDate: string): boolean {
    const date = new Date(rawDate);
...
function openApplicationModal(jobId: string): void {
...
function attachApplicationHandlers(): void {
...
function toggleSaved(jobId: string): void {
    if (isJobSaved(jobId)) {
...
function renderSavedJobs(): void {
    ...
    if (elements.savedCount) elements.savedCount.textContent = String(savedJobs.length);
    if (savedJobs.length === 0) {
        savedList.innerHTML = '<div class="empty-state">No saved jobs yet. Click Save on a role to bookmark it.</div>';
        return;
    }
...
function updateJobTypeButtons(): void {
    if (!elements.jobTypeButtons) return;
    elements.jobTypeButtons.querySelectorAll('.type-button').forEach((button) => {
        const btn = button as HTMLElement;
        if (btn.dataset.type === state.jobType) {
...
function resetFilters(): void {
    ...
    if (elements.companySelector) {
        const companyDisplay = elements.companySelector.querySelector('.select-value') as HTMLElement;
        if (companyDisplay)
            companyDisplay.textContent = 'All companies';
    }
    if (elements.roleSelector) {
        const roleDisplay = elements.roleSelector.querySelector('.select-value') as HTMLElement;
        if (roleDisplay)
            roleDisplay.textContent = 'All roles';
    }
    if (elements.postedSelect) elements.postedSelect.value = 'all';
...
function attachControls(): void {
    if (!elements.keywordInput || !elements.companySelector || !elements.roleSelector || !elements.postedSelect || !elements.sortSelect || !elements.jobTypeButtons || !elements.clearFilters) return;
    elements.keywordInput.addEventListener('input', (event) => {
        state.keyword = (event.target as HTMLInputElement).value.trim();
        renderJobs();
    });
    ...
    elements.postedSelect.addEventListener('change', (event) => {
        state.postedRange = (event.target as HTMLSelectElement).value;
        renderJobs();
    });
    elements.sortSelect.addEventListener('focus', () => hideDropdown());
    elements.sortSelect.addEventListener('change', (event) => {
        state.sortBy = (event.target as HTMLSelectElement).value;
        renderJobs();
    });
    elements.jobTypeButtons.querySelectorAll('.type-button').forEach((button) => {
        button.addEventListener('click', () => {
            const btn = button as HTMLElement;
            state.jobType = btn.dataset.type || 'All';
            updateJobTypeButtons();
            renderJobs();
            hideDropdown();
        });
